...
class ProductActivityInlineChartWidget extends InlineChartWidget
{
    protected static ?string $maxHeight = '50';
    protected int | string | array $columnSpan = 'full';
    protected static ?string $heading = 'Product Activity';
    ...
    public function getData(): array
    {
        ...
        $transactions = \App\Models\Transaction::where('product_id', $this->record->id)
            ->whereBetween('created_at', [now()->subDays(7), now()])
            ->selectRaw('DATE(created_at) as date, COUNT(*) as count')
            ->groupBy('date')
            ->get()
